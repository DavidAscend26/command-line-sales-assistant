...
export type Route =
  ...
  | { kind: "deal_status"; dealId: DealId }
  | { kind: "draft_email"; dealId: DealId; intent: string }
  | { kind: "unknown"; reason: string };

function extractDealId(text: string): DealId | null {
  const m = text.match(/\bD-(103|221)\b/i);
  if (!m) return null;
  const normalized = `D-${m[1]}` as DealId;
  return normalized;
}

export function routePrompt(prompt: string): Route[] {
  ...
  if ((p.includes("status") || p.includes("stage") || p.includes("deal")) && dealId) {
    const routes: Route[] = [{ kind: "deal_status", dealId }];
